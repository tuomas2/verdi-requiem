...
| Movement I's chorus S/A/T carry OMR garble the text dump now names exactly (`lg},`, `ux`, `per-pe-tll-a`, `In-ce-at`, the editor's name `Reutenauer` at bar 68). One `korjaa_kasin.py` row each once someone reads the source page | *2026-09-09*, section *What is left here*; the list is `python3 suomennos.py`'s own output |
...
- `python3 -m unittest discover -s testit -t .` — 247 tests.
...
### Reading the words is harder than it looks, and getting it wrong is worse
### than not doing it
