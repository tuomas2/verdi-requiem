...
| Movement 01's chorus **bass, bar 40 is half a bar long**: one half note `B♭3` on "ex" and **no half rest before it**, so 2/4 of a 4/4 bar. The choir file has the rest, which puts the "ex-au-di" entry on beat 3; in the practice `.mscz` ours sounds two beats early. Found 2026-09-10 (c) as a by-product of the piano bar-mapping, and missed by the bars 1–78 check because that compared pitches, not rhythm. Fixing it needs a new `korjaa_kasin.py` operation — there is no "insert a rest" yet | [`TODO.md`](TODO.md); [`2026-09-10-nuottikirjan-nelja-tarppia.md`](docs/tyopaivakirja/2026-09-10-nuottikirjan-nelja-tarppia.md) |
...
| Movement I has no piano — and **the choir files have one** (`01_requiem`: 1620 notes over 127 bars; `04_dies_irae_2`: 1739 over 96). The user's idea 2026-09-10, recorded and not started: copy the piano out of the choir files where they cover a movement, both to fill this gap and to turn the piano into an *independent* arbiter for note questions — which is exactly what it is not today, since ours comes out of the same OMR pass as the voices. The **bar mapping is measured** (2026-09-10 (c)): movement 01 is two constant offsets and movement 10b one, both proven bar by bar; movement 14 is three chunks and the middle one is anchored by only two bars. Movement I's piano also turns out to be **rejected, not missing** — 1912 OMR notes that crash MuseScore, so `P17` is left out of `MAPPING` — which makes the choir's piano a replacement for a known-bad part rather than a gap-filler. What is left is the graft itself (divisions 12 vs our 4, so a ×3 rescale), the 11-bar hole at our 79–89 where the choir cut the soloists' passage, and asking about provenance before unknown-authorship material goes into a public repo | [`docs/menetelmat/yhdistaminen.md`](docs/menetelmat/yhdistaminen.md), *Movement I has no piano*; [`docs/menetelmat/kuorotiedostot.md`](docs/menetelmat/kuorotiedostot.md); [`TODO.md`](TODO.md), *Isoimmat voitot* |
