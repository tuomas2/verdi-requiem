...
|---|---|
| **Notes** of movement 01: only bars 1–78 are done (all four chorus voices, against the choir file and the printed page); its **Kyrie, bars 79–140, is not** — but the bar mapping that blocked this is **measured 2026-09-10 (c)**: our 1–78 ↔ the choir file's 1–78, and our 90–138 ↔ its 79–127, a clean **+11** because the choir cut our 79–89 (the soloists' passage). At that offset the Kyrie leaves **13 candidate bars** (96, 103, 105, 115–118, 120–121, 123–126) that are now examinable rather than noise. Movement 14's `Kuoro B` is done; its S/A/T are not | [`2026-09-10-kuorotiedostot-ja-osan-i-savelet.md`](docs/tyopaivakirja/2026-09-10-kuorotiedostot-ja-osan-i-savelet.md); [`docs/menetelmat/omr.md`](docs/menetelmat/omr.md) |
| Movement 01's chorus-bass **text** now reads complete (86 % of its notes carry a syllable; the 15 that do not are melisma-internal). Its **notes** are now checked against the choir file for bars 1–78 (two differences, both resolved in our favour by the printed page, and the printed book confirmed one of them on 2026-09-10 along with bars 51–52's `om-nis`, which went back to the source page's placement). **Bar 40's missing half rest** was found the same day and fixed with a new `lisaa_tauko` operation — the bar was 2/4 long and the "ex-au-di" entry sounded two beats early; the pitch-by-pitch check could not see it, which is worth remembering before calling a range done. S/A/T got their first text work 2026-09-10 — the line "et lux per-pe-tu-a", and the soprano's whole "Te decet hymnus" lifted off lyric row 2 — but are still far from proofread | [`2026-09-02-tahtinumerot-ja-osa-i.md`](docs/tyopaivakirja/2026-09-02-tahtinumerot-ja-osa-i.md), [`2026-09-10-kuorotiedostot-ja-osan-i-savelet.md`](docs/tyopaivakirja/2026-09-10-kuorotiedostot-ja-osan-i-savelet.md) |
...
| Movement 14's `Kuoro B`: its **notes** are settled — they match the choir file exactly, all 74 bars are metrically valid — but its **words are not**, and the ✔ came off on 2026-09-10. The choir's files carry no lyrics, so the only check ever run on them was "does every note carry a syllable", which cannot see a *wrong* syllable; the singer heard one in bars 40–42, where the machine had copied the upper voices' "do-na, do-na" onto a bass line that has one melisma there. What is left here: **read the bass's lyric row off all five source pages and compare it syllable by syllable**. That is started and stopped — the blocker is naming the chorus bass staff per system, since a system has two, four or six staves depending on who is singing, and on page 4 the bottom system's vocal staves have no clef glyph in the text layer at all. Its **Soprano/Alto/Tenor** are further behind still: 48–59 % of their notes carry no syllable, and they show fabricated content and contentless measures in bars 59–74 | [`2026-09-10-agnus-dein-yksi-melisma.md`](docs/tyopaivakirja/2026-09-10-agnus-dein-yksi-melisma.md); [`2026-08-31-agnus-dei-kuorobasso.md`](docs/tyopaivakirja/2026-08-31-agnus-dei-kuorobasso.md) |
...
| Movement I's **piano is in** since 2026-09-10, taken from the choir's own MuseScore file by `kuoropiano.py` — 126 of 140 bars, our 79–90 and 139–140 left as rests because the choir cut the soloists' passage. What is left here: those 14 silent bars, and **movements 10b and 14, which are mapped but not copied** — 10b because its two pianos disagree by an octave in six bars (five of them the opening figure's right hand), 14 because its gain is smaller. Also open: **`yhdista.py` reads `divisions` from one reference part and uses it for every row**, which is what made `mscore` refuse the whole score here; `tarkista_jaotus` now reports the mismatch in ten source parts and `tarkista_mitat` counts 552 wrong-length bars in the finished score, both pre-existing and neither chased | [`docs/menetelmat/kuorotiedostot.md`](docs/menetelmat/kuorotiedostot.md), *The piano in these files*; [`docs/menetelmat/yhdistaminen.md`](docs/menetelmat/yhdistaminen.md); [`2026-09-10-osan-i-piano.md`](docs/tyopaivakirja/2026-09-10-osan-i-piano.md) |
...
Tests, from the repo root: `python3 -m unittest discover -s testit -t .` (439).
